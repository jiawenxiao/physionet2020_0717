...
def train_epoch(model, optimizer, criterion,x_train,x_train_external,y_train,num_class):
    ...
    true_train=torch.zeros((0,num_class),dtype=torch.float,device=device)
    pred_train=torch.zeros((0,num_class),dtype=torch.float,device=device)
    ...
    for i in range(0,len(x_train)-batch_size,batch_size):      
        ...
        true_train=torch.cat((true_train, target),dim=0)
        pred_train=torch.cat((pred_train,torch.sigmoid(output)),dim=0)            
    auc_meter = utils.calc_auc( true_train, pred_train)    
    return loss_meter / it_count, auc_meter
...
def val_epoch(model, criterion, x_val,x_val_external,y_val,num_class):
    ...
    true_val=torch.zeros((0,num_class),dtype=torch.float,device=device)
    pred_val=torch.zeros((0,num_class),dtype=torch.float,device=device)
    with torch.no_grad():
        for i in range(0,len(x_val)-batch_size,batch_size):      
            ...
            loss_meter += loss.item()
            it_count += 1
            true_val=torch.cat((true_val, target),dim=0)
            pred_val=torch.cat((pred_val,torch.sigmoid(output)),dim=0)              
    auc_meter = utils.calc_auc( true_val, pred_val)           
    return loss_meter / it_count, auc_meter
